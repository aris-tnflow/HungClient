...
export const FilterSelect = ({ dataIndex, handleTableChange, options }) => {
  const handleSearch = (selectedValue, confirm) => {
    confirm();
    if (handleTableChange && typeof handleTableChange === "function") {
      handleTableChange({
...
export const FilterSelectArray = (dataIndex, options) => {
  return {
    ...
    onFilter: (value, record) => {
      const isInArray =
        Array.isArray(record[dataIndex]) && record[dataIndex].includes(value);
      return isInArray;
    },
    filterSearch: true,
  };

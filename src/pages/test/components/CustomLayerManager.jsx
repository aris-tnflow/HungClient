...
const getDragTarget = (ev) => {
  ...
  return {
    el: elLayer,
    cmp: (elLayer && elLayer.__cmp) || null,
  };
};

export default function CustomLayerManager({ root }) {
  const editor = useEditor();
  const [pointerDown, setPointerDown] = useState(false);
  const [canMoveRes, setCanMoveRes] = useState({});
  const [cmpPointerOver, setCmpPointerOver] = useState(null);
  const [dragging, setDragging] = useState(null);
  const [dragParent, setDragParent] = useState(null);
